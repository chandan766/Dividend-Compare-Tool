...
def create_database_and_table():
    conn = sqlite3.connect('dividend.db')
    cursor = conn.cursor()
    
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS dividend (
            companyName VARCHAR(50),
            sharePrice VARCHAR(15),
            annualDividend VARCHAR(15),
            shares VARCHAR(15)
        )
    ''')
    conn.commit()
...
def save():
    global save_flag
    if len(CompName)>0 and save_flag==False:
        con = sqlite3.connect('dividend.db')
        cur = con.cursor()
        ...
        messagebox.showerror("Error!","An error occurred while saving the data. Please try again later or check the table carefully!")
